...
pub fn encrypt_for_peer(recipient_public_key: &[u8], plaintext: &[u8]) -> Result<Vec<u8>, EchoMeshError> {
    if recipient_public_key.len() != 32 {
        return Err(EchoMeshError::InvalidKeyLength {
            expected: 32,
            actual: recipient_public_key.len() as u32,
        });
    }
    ...
    hk.expand(HKDF_INFO, &mut key)
        .map_err(|_| EchoMeshError::CryptoError("HKDF expansion failed".into()))?;

    let cipher = ChaCha20Poly1305::new_from_slice(&key)
        .map_err(|_| EchoMeshError::CryptoError("invalid AEAD key".into()))?;
    let mut nonce_bytes = [0u8; NONCE_LEN];
    OsRng.fill_bytes(&mut nonce_bytes);
    let ciphertext = cipher
        .encrypt(Nonce::from_slice(&nonce_bytes), plaintext)
        .map_err(|_| EchoMeshError::CryptoError("E2EE encryption failed".into()))?;
...
pub fn decrypt_from_peer(local_private_key: &[u8], envelope: &[u8]) -> Result<Vec<u8>, EchoMeshError> {
    ...
    cipher
        .decrypt(
            Nonce::from_slice(&envelope[33..45]),
            &envelope[HEADER_LEN..],
        )
        .map_err(|_| EchoMeshError::CryptoError("E2EE authentication failed".into()))
}
