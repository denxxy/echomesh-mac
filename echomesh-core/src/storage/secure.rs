...
impl StorageManager {
    /// Opens an encrypted SQLCipher database. Existing plaintext EchoMesh
    /// databases are migrated in-place through sqlcipher_export before use.
    ...
    /// In-memory storage for unit tests. It uses the same schema but no durable
    /// pages exist to protect on disk.
    ...
    pub fn get_contacts(&self) -> Result<Vec<Contact>, EchoMeshError> {
        ...
        rows.collect::<Result<Vec<_>, _>>()
            .map_err(storage_err("read contacts"))
    }
    ...
    pub fn get_contact_by_peer_id(&self, peer_id: &[u8]) -> Result<Option<Contact>, EchoMeshError> {
        ...
        .optional()
        .map_err(storage_err("get contact"))
    }
    ...
    pub fn save_message(&self, msg: &MessageRecord) -> Result<(), EchoMeshError> {
        ...
        conn.execute(
            "INSERT INTO messages
             (id, conversation_peer_id, sender_peer_id, text, timestamp, is_outgoing, status)
    ...
    pub fn get_messages(
        ...
        let mut stmt = conn
            .prepare(
                "SELECT id, conversation_peer_id, sender_peer_id, text, timestamp, is_outgoing, status
                 FROM messages WHERE conversation_peer_id = ?1
        ...
        let rows = stmt
            .query_map(params![peer_id, limit as i64, offset as i64], |row| {
                Ok(MessageRecord {
                    id: row.get(0)?,
                    conversation_peer_id: row.get(1)?,
        ...
        rows.collect::<Result<Vec<_>, _>>()
            .map_err(storage_err("read messages"))
    }
    ...
    pub fn get_conversations(&self) -> Result<Vec<ConversationSummary>, EchoMeshError> {
        ...
        rows.collect::<Result<Vec<_>, _>>()
            .map_err(storage_err("read conversations"))
    }
}
...
fn verify_sqlcipher(conn: &Connection) -> Result<(), EchoMeshError> {
    ...
    if version.trim().is_empty() {
        return Err(EchoMeshError::StorageError(
            "SQLCipher support is unavailable".to_string(),
        ));
    }
...
mod tests {
    ...
    fn in_memory_schema_round_trip() {
        ...
        assert!(contacts.iter().any(|c| c.peer_id == ECHO_SERVICE_PEER_ID));
    }
